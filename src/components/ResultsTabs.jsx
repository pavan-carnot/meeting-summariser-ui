...
export default function ResultsTabs({ activeView, onNavigate, hasSummary }) {
  return (
    <div style={{
      display: 'flex', gap: 4, borderBottom: '1px solid #e5e2d6',
...
function Tab({ icon: Icon, label, active, disabled, onClick }) {
  ...
  const fg = active ? '#0A0F1E' : disabled ? '#c9c5b5' : (hover ? '#0A0F1E' : '#66645c');
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      ...
      style={{
        ...
        color: fg,
        borderBottom: active ? '2.5px solid #00C5B0' : '2.5px solid transparent',
        marginBottom: -1,
